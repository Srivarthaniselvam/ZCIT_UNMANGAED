CLASS lsc_ZCIT_I_HDR_AD303 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZCIT_I_HDR_AD303 IMPLEMENTATION.
  ...
  METHOD save.
    ...
    DATA(lo_util) = zcit_util_22ad303=>get_instance( ).
    ...
    IF ls_sales_hdr IS NOT INITIAL.
      MODIFY zcit_so_hdr_303 FROM @ls_sales_hdr.
    ENDIF.

    " 2. Save/Update Item to your database table
    IF ls_sales_itm IS NOT INITIAL.
      MODIFY zcit_so_itm_303 FROM @ls_sales_itm.
    ENDIF.

    " 3. Handle Deletions based on buffer flags
    IF lv_so_hdr_del = abap_true.
      LOOP AT lt_sales_header INTO DATA(ls_del_hdr).
        DELETE FROM zcit_so_hdr_303 WHERE salesdocument = @ls_del_hdr-salesdocument.
        DELETE FROM zcit_so_itm_303 WHERE salesdocument = @ls_del_hdr-salesdocument.
      ENDLOOP.
    ELSE.
      ...
      LOOP AT lt_sales_items INTO DATA(ls_del_itm).
        DELETE FROM zcit_so_itm_303 WHERE salesdocument = @ls_del_itm-salesdocument
  ...
  METHOD cleanup.
    ...
    zcit_util_22ad303=>get_instance( )->cleanup_buffer( ).
  ENDMETHOD.
